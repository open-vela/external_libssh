                       ...
                       bool private)
{
    ...
    SSH_LOG(SSH_LOG_INFO,
            ...
            nkeys);
    if (nkeys != 1) {
        SSH_LOG(SSH_LOG_WARN, "Opening OpenSSH private key: only 1 key supported (%"PRId32" available)", nkeys);
        goto out;
    }
                                   ...
                                   ssh_string salt)
{
    ...
    SSH_LOG(SSH_LOG_WARN, "Encryption: %d key, %d IV, %"PRId32" rounds, %zu bytes salt",
                cipher.keysize/8,
